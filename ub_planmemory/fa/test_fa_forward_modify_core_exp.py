...
@triton.jit
def _attn_fwd(Q, K, V, ATTEN_MASK, M, Out, sm_scale: tl.constexpr, sparse_start_idx, #
    ...
    for block_idx in range(start_block, end_block, step):
        ...
        if STAGE & 1:
            acc, l_i, m_i = _attn_fwd_inner(acc, l_i, m_i, q, K_block_ptr, V_block_ptr, ATTEN_MASK, #
...
class _attention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, atten_mask, causal, sm_scale, BM, BN):
        # shape constraints
